...
export default function PokeCard(props) {
    ...
    const imgList = Object.keys(sprites || {}).filter((value) => {
        return !['versions', 'other'].includes(value) && sprites[value];
    })
    ...
    }, [selectedPokemon]); // whenever the user selects another pokemon, we're going to need to refetch the info for the new pokemon
    ...
    return (
        <div className='poke-card'>
            { /* if skill is true, render the Modal, else dont. */ }
            {skill && (
            ...
            <img className='default-img' src={'/pokemon/' + getFullPokedexNumber(selectedPokemon) + '.png'} alt={`*{name}-large-img`} /> { /* we dont have to access the public directory cuz the public directory is the default folder that searches for assets */ }
                    ...
                    stats.map((statObj, statIndex) => {
                        ...
                        return (
                            <div key={statIndex} className='stat-item'>
                                <p>{stat?.name.replaceAll('-', ' ')}</p> { /* replaceAll() to replace all the - with empty space */}
                                <h4>{base_stat}</h4>
                            </div>
                        )
                    })
